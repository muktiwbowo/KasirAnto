...
public class ActivityTiketMasukUpdate extends AppCompatActivity implements View.OnClickListener {
    ...
    private void updateKolamIkan(String id, String nama, String harga){
        retrofit2.Call<AdminMessage> call = RestAPIHelper.ServiceApi(getApplication()).updateTiketMasuk(id,nama,harga);
        call.enqueue(new Callback<AdminMessage>() {
            ...
            public void onResponse(Call<AdminMessage> call, Response<AdminMessage> response) {
                if (response.body() != null){
                    ...
                    if (!error){
                        Toast.makeText(ActivityTiketMasukUpdate.this, msg, Toast.LENGTH_SHORT).show();
                    }else {
                        Toast.makeText(ActivityTiketMasukUpdate.this, msg, Toast.LENGTH_SHORT).show();
                    }
                }
            ...
            public void onFailure(Call<AdminMessage> call, Throwable t) {
                Toast.makeText(ActivityTiketMasukUpdate.this, "Cek koneksi internet anda", Toast.LENGTH_SHORT).show();            }
        });
    }

    private void deleteKolamIkan(String id){
        retrofit2.Call<AdminMessage> call = RestAPIHelper.ServiceApi(getApplication()).deleteTiketMasuk(id);
        call.enqueue(new Callback<AdminMessage>() {
            @Override
            public void onResponse(Call<AdminMessage> call, Response<AdminMessage> response) {
                if (response.body() != null){
