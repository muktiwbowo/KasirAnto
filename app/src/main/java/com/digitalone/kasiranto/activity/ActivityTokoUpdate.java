...
public class ActivityTokoUpdate extends AppCompatActivity implements View.OnClickListener {
    ...
    private void updateToko(String id, String nama, String harga, String stok){
        retrofit2.Call<AdminMessage> call = RestAPIHelper.ServiceApi(getApplication()).updateToko(id,nama,harga,stok);
        call.enqueue(new Callback<AdminMessage>() {
            ...
            public void onResponse(Call<AdminMessage> call, Response<AdminMessage> response) {
                if (response.body() != null){
                    ...
                    if (!error){
                        Toast.makeText(ActivityTokoUpdate.this, msg, Toast.LENGTH_SHORT).show();
                    }else {
                        Toast.makeText(ActivityTokoUpdate.this, msg, Toast.LENGTH_SHORT).show();
                    }
                }
            ...
            public void onFailure(Call<AdminMessage> call, Throwable t) {
                Toast.makeText(ActivityTokoUpdate.this, "Cek koneksi internet anda", Toast.LENGTH_SHORT).show();            }
        });
    }

    private void deleteToko(String id){
        retrofit2.Call<AdminMessage> call = RestAPIHelper.ServiceApi(getApplication()).deleteToko(id);
        call.enqueue(new Callback<AdminMessage>() {
            @Override
            public void onResponse(Call<AdminMessage> call, Response<AdminMessage> response) {
                if (response.body() != null){
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.toko_btn_update:
                updateToko(edtId.getText().toString(), edtNama.getText().toString(),
                        edtHarga.getText().toString(), edtStok.getText().toString());
                break;
            case R.id.toko_btn_hapus:
                deleteToko(edtId.getText().toString());
                break;
        }
    }
